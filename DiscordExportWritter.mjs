...
export class DiscordExportWritter {
    ...
    async downloadChannelOrThread(channel, ignoreChannelIds, lastMessageIds, OUTPUT_FOLDER) {
        ...
        if (this.getBlacklistedLastMessageIds().includes(channel.last_message_id)) {
            ...
            await this.execCommand(`DiscordChatExporter.Cli export --token ${this.token} --format Json --media --reuse-media --fuck-russia --markdown false --channel ${channel.id} --output ${OUTPUT_FOLDER}`, channel.last_message_id);
            ...
            await this.execCommand(`DiscordChatExporter.Cli export --token ${this.token} --format Json --media --reuse-media --fuck-russia --markdown false --channel ${channel.id} --after ${moment(lastMessageIds[channel.id]['timestamp']).utcOffset(0).add(1, 'seconds').format()} --output ${OUTPUT_FOLDER}`, channel.last_message_id);
            ignoreChannelIds.push(channel.id);
        }
